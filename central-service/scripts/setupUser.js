...
async function setupUser() {
    ...
    if (!newDbUser || !newDbPassword) {
        console.error('🚨 [CentralService][UserSetup].env 파일에 PGUSER 또는 PGPASSWORD 설정 누락. 스크립트 종료.');
        return;
    }
    ...
    try {
        ...
        console.log(`🔌 [CentralService][UserSetup] PostgreSQL (${process.env.DFPGDATABASE} DB) 최고 관리자 연결 성공`);
        ...
        if (rows.length > 0) {
            // 이미 사용자가 존재하는 경우
            console.warn(`🔔 [CentralService][UserSetup] 사용자 '${newDbUser}' 이미 존재.`);
        } else {
            // 4. 사용자가 존재하지 않는 경우, 새로 생성합니다.
            console.log(`🚀 [CentralService][UserSetup] 사용자 '${newDbUser}' 생성 시작...`);
            ...
            console.log(`✅ [CentralService][UserSetup] 사용자 '${newDbUser}' 생성 완료.`);
        }

    } catch (err) {

        console.error(`🚨 [CentralService][UserSetup] 사용자 설정 중 오류 발생: ${err.message}`);
        process.exit(1);
        ...
        if (client) {
            await client.end();
            console.log(`🔌 [CentralService][UserSetup] PostgreSQL 연결 종료.`);
        }        
