...
async function createDatabase() {
    ...
    if (!newDbName || !newDbUser) {
        console.error('🚨 [ExternalService][DBCreate] .env 파일에 PGDATABASE 또는 PGUSER 설정 누락. 스크립트 종료.');
        return;
    }
    ...
    try {        
        ...
        console.log(`🔌 [ExternalService][DBCreate] PostgreSQL (${process.env.DFPGDATABASE} DB) 최고 관리자 연결 성공`);
        ...
        if (rows.length > 0) {
            // 이미 데이터베이스가 존재하는 경우
            console.warn(`🔔 [ExternalService][DBCreate] 데이터베이스 '${newDbName}' 이미 존재.`);
        } else {
            // 4. 데이터베이스가 존재하지 않는 경우, 새로 생성합니다.
            console.log(`🚀 [ExternalService][DBCreate] 데이터베이스 '${newDbName}'을(를) 생성 시작...`);
            ...
            console.log(`✅ [ExternalService][DBCreate] 데이터베이스 '${newDbName}' 생성 완료 (소유자: ${newDbUser}).`);

        }

    } catch (err) {

        console.error(`🚨 [ExternalService][DBCreate] 데이터베이스 생성 중 오류 발생: ${err.message}`);
        process.exit(1);
        ...
        await client.end();
        console.log(`🔌 [ExternalService][DBCreate] PostgreSQL 연결 종료.`);
