...
pool.on('connect', (client) => {
    logger.debug('🔌 [ExternalService][DBPool] 새 DB 클라이언트 연결 완료.');
});

// 커넥션 풀에서 오류가 발생했을 때 로그를 기록합니다.
pool.on('error', (err, client) => {
    logger.error('🚨 [ExternalService][DBPool] DB 커넥션 풀 예기치 않은 오류 발생.', err);
});

// 커넥션 풀에서 클라이언트가 제거(연결 종료)될 때 로그를 남깁니다.
pool.on('remove', (client) => {
        logger.debug('✅ [ExternalService][DBPool] DB 클라이언트가 풀에서 제거 완료.');
});
...
const query = async (text, params) => {
    ...
    try {
        ...
        // 쿼리 로그가 너무 길어지지 않도록 첫 줄만 잘라서 보여줍니다.
        logger.debug(`✅ [ExternalService][DBQuery] 쿼리 실행 완료 (${duration}ms): ${text}.`);
        return result;

    } catch (err) {

        logger.error(`🚨 [ExternalService][DBQuery] 쿼리 실행 중 오류 발생: ${text}\n${err.stack}`);
...
const getClient = async () => {
    logger.debug('[ExternalService][DBPool] 클라이언트 요청.');
    const client = await pool.connect();
    logger.debug('✅ [ExternalService][DBPool] 클라이언트 획득 완료.');
    return client;
};
...
const disconnect = async () => {
    ...
    logger.info('🔌 [ExternalService][DBPool] DB 커넥션 풀 종료 시작.');
    await pool.end();  
    logger.info('✅ [ExternalService][DBPool] DB 커넥션 풀 종료 완료.');
